...
class WeeklyTaxiShiftsController extends Controller
{
    ...
    public function printPDF($date)
    {
          if($this->checkIsAValidDate($date)){
            ...
            return redirect(route('director.reports.WeeklyTaxiShifts.index'))->with('success','date is incorrect');
          }
      
      
          if(count($shifts)<1)
          {
          return redirect(route('director.reports.WeeklyTaxiShifts.index'))->with('success','No shifts found for the selected week');
          }


          set_time_limit(1000);
          // return view('director.reports.WeeklyTaxiShifts.PDF_WeeklyTaxiShifts',compact('shifts','dateData','specialShifts')); 
          $pdf = PDF::loadView('director.reports.WeeklyTaxiShifts.PDF_WeeklyTaxiShifts',compact('shifts','date','specialShifts','director_comments'))->setPaper('a3', 'landscape'); 
          $name = 'Weekly taxi shifts report - '.$date.'.pdf';
          return $pdf->stream($name,array('Attachment'=>0));
          return redirect(route('director.reports.WeeklyTaxiShifts.index'))->with('success','pdf is generating');
    }
