...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        $dispatcher = Dispatcher::where('email', $request->email)->first();

        if ($dispatcher) {
            if (Hash::check($request->password, $dispatcher->password)) {
                if (Hash::check("taxi-office", $dispatcher->password)) {

                    $tokenResult = $dispatcher->createToken('Personal Access Token');
