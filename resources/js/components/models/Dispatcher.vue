<template>
  <div class="text-center">
    <v-dialog v-model="showDispatcherLogin" width="450">
      <v-card>
        <v-toolbar color="info" dark flat>
          <v-icon class="mr-5">verified_user</v-icon>

          <v-toolbar-title>Dispatcher Login</v-toolbar-title>
...
export default {
  ...
  computed: {
    showDispatcherLogin: {
      get() {
        return this.$store.state.showDispatcherLogin;
      },
      set(newval) {
        if (newval === true) {
          this.$store.commit("showDispatcherLogin_on");
        } else {
          this.$store.commit("showDispatcherLogin_off");
        }
      }
    }
  },
  methods: {
    showDispatcherLogin_on: function() {
      this.$store.commit(showDispatcherLogin_on);
    },
    showDispatcherLogin_off: function() {
      this.$store.commit(showDispatcherLogin_off);
    },
    login() {
      this.loading = true;
      ...
      this.$store
        ...
        .then(res => {
          this.loading = false;
          this.showDispatcherLogin = false;
          if (!res.data.changePassword) {
            this.$swal(
              "welcome !",
              "you were logged in successfully as a DISPATCHER!",
              "success"
            );
