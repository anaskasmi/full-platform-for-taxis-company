...
export default {
  ...
  data() {
    return {
      items: [
        { title: "Profile", icon: "person",link:{ name: 'AdminDashboard_profile'} },
        { title: "Change Passowrd", icon: "vpn_key",link:{ name: 'AdminDashboard_changePassword'} }
      ],
      right: null,
      role: "Admin",
      name: "",
  ...
  computed: {
    show() {
      return this.$store.state.showDrawerAdmin;
    }
  },
  methods: {
    BASE_URL()
    ...
    fetchItems() {
      ...
      axios
        .get(this.BASE_URL()+"/api/admin/auth/user")
        .then(res => {
