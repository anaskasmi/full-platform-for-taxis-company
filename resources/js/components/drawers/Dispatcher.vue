...
export default {
  ...
  data() {
    return {
      items: [
        { title: "Profile", icon: "person",link:{ name: 'DispatcherDashboard_profile'} },
        { title: "Change Passowrd", icon: "vpn_key",link:{ name: 'DispatcherDashboard_changePassword'} }
      ],
      right: null,
      role: "Dispatcher",
      name: "",
  ...
  computed: {
    show() {
      return this.$store.state.showDrawerDispatcher;
    }
  },
  methods: {
    BASE_URL()
    ...
    fetchItems() {
      ...
      axios
        .get(this.BASE_URL()+"/api/dispatcher/auth/user")
        .then(res => {
