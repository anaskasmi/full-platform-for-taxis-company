                ...
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tr v-for="(specialShift,i) in specialShifts" :key="i">
                            ...
                            <td class="text-right">
                                <v-icon color="success" class="mx-1" @click="editSpecialShift(specialShift)">edit
                                </v-icon>
                                ...
                                <v-icon color="info" class="mx-1" @click="showSpecialShift(specialShift)">description
                                </v-icon>
                            </td>
                        </tr>
                        </tbody>
                ...
                <div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-striped">
                            ...
                            <tbody>
                            <tr v-for="(specialShift,i) in specialShifts" :key="i">
                                <td class="align-middle fitCell">{{specialShift.FirstName}} {{specialShift.FirstName}}
                                </td>
                                <td class="align-middle fitCell">{{specialShift.PermitNumber}}</td>
                                <td class="align-middle fitCell">{{specialShift.ShiftType}}</td>
                                <!-- if open shift -->
                                <td class="align-middle fitCell" v-if="specialShift.ShiftType =='openshift'">
                                    {{specialShift.Car}}
                                </td>
                                <td class="align-middle fitCell" v-if="specialShift.ShiftType =='openshift'">-</td>
                                <td class="align-middle fitCell" v-if="specialShift.ShiftType =='openshift'">-</td>
                                <!-- not open shift -->
                                <td class="align-middle fitCell" v-if="specialShift.ShiftType !='openshift'">-</td>
                                <td class="align-middle fitCell" v-if="specialShift.ShiftType !='openshift'">
                                ...
                                <td class="text-right">
                                    <v-icon color="success" class="ma-4" @click="editSpecialShift(specialShift)">edit
                                    </v-icon>
                                    ...
                                    <v-icon color="info" class="ma-4" @click="showSpecialShift(specialShift)">
                                        description
                                    </v-icon>
                                </td>
                            </tr>
        ...
        methods: {
            ...
            search(searchValue) {
                ...
                axios
                    .get(
                        this.BASE_URL() +
                        "/api/dispatcher/mySpecialShifts/searchMySpecialShifts?searchValue=" +
                        searchValue
                    )
                    .then(res => {
            ...
            fetchItems(page_url) {
                let vm = this;
                page_url = page_url || this.BASE_URL() + "/api/dispatcher/mySpecialShifts";
                axios.defaults.headers.common["Authorization"] =
                    "Bearer " + this.$store.state.token_dispatcher;
                axios
                    .get(page_url)
            ...
            next(page) {
                this.fetchItems(
                    this.BASE_URL() + "/api/dispatcher/mySpecialShifts?page=" + page
                );
            },
