<template>
  <div>
    ...
    <div class="row col-12">
      <v-btn color="grey" text tile class="float-left" :to="tripRoute">
        <v-icon class="mr-2">keyboard_return</v-icon>Trip
      </v-btn>
    </div>
    <hr />
    <!-- errors  -->
    ...
    <form id="form" @submit.prevent="editTrip">
      <v-alert
        border="left"
        tile
        dense
      ...
      <div class="form-group">
        <v-text-field label="FARE" v-model="trip.fare" type="text" name="fare"></v-text-field>
      </div>

      <div class="form-group">
        <v-text-field label="TIP" v-model="trip.tip" type="text" name="tip"></v-text-field>
      </div>
      ...
      <div class="form-group">
        <v-text-field label="PICKUP ADDRESS" v-model="trip.pickup" type="text" name="pickup"></v-text-field>
      </div>

      <div class="form-group">
        <v-text-field label="DROP OFF ADDRESS" v-model="trip.dropoff" type="text" name="dropoff"></v-text-field>
      </div>
...
export default {
  created: function() {
    ...
    axios
      .get(this.BASE_URL + "/api/driver/trip/" + this.$route.params.id)
      .then(res => {
  ...
  methods: {
    ...
    editTrip() {
      ...
      axios.defaults.headers.common["Authorization"] =
        "Bearer " + this.$store.state.token_driver;
      axios
        .post(this.BASE_URL()+"/api/driver/trip/" + this.trip.trip_id, formData)
        .then(response => {
          console.log("the response of edit is :");
          console.log(response);
          this.$router.push({
            name: "DriverDashboard_trips_show",
            params: { id: this.trip.trip_id }
          });
        })
        .catch(error => {
          console.log("catch");
