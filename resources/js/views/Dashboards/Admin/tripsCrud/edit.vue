  ...
  <div>
    ...
    <div class="row col-12">
      <v-btn color="grey" text tile class="float-left" :to="tripsRoute">
        <v-icon class="mr-2">keyboard_return</v-icon>trips
      </v-btn>
    </div>
    <hr />
    <!-- errors  -->
    ...
    <form id="form" @submit.prevent="editTrip">
      ...
      <div class="form-group">
        <v-text-field label="Badge Id" v-model="trip.badge_id" type="text" name="badge_id"></v-text-field>
      </div>
...
export default {
  created: function() {
    ...
    axios
      .get(this.BASE_URL() + "/api/admin/trip/" + this.id)
      .then(res => {
  ...
  methods: {
    ...
    editTrip() {
      ...
      axios.defaults.headers.common["Authorization"] =
        "Bearer " + this.$store.state.token_admin;
      axios
        .post(this.BASE_URL()+"/api/admin/trip/" + this.id, formData)
        .then(response => {
